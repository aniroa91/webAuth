...
object ProblemService extends AbstractService{
  ...
  def listProvinceByWeek(week: String): Future[Seq[(String, String, Long)]] = {
    ...
    dbConfig.db.run(
      sql"""select province, device_type, sum(n_prob_device) from dmt_weekly_mapping where week >= $startDate::TIMESTAMP AND week <= $endDate::TIMESTAMP
  ...
  def listProbconnectivity(week: String): Future[Seq[(String, Long, Long)]] = {
    ...
    val endDate = week.split("->")(1).trim
    dbConfig.db.run(
      sql"""select bras_id, sum(signin) signin, sum(logoff) logoff from dmt_weekly_conn where week >= $startDate::TIMESTAMP AND week <= $endDate::TIMESTAMP AND flag_signin =1 OR flag_logoff = 1
            group by bras_id
  ...
  def listProbError(week: String): Future[Seq[(String, Long)]] = {
    ...
    val endDate = week.split("->")(1).trim
    dbConfig.db.run(
      sql"""select bras_id, sum(err) err from dmt_weekly_kib where week >= $startDate::TIMESTAMP AND week <= $endDate::TIMESTAMP AND flag_err =1 OR flag_cluster = 1
            group by bras_id
  ...
  def listProbWarning(week: String): Future[Seq[(String, Long)]] = {
    ...
    val endDate = week.split("->")(1).trim
    dbConfig.db.run(
      sql"""select bras_id, sum(warn) warn from dmt_weekly_kib where week >= $startDate::TIMESTAMP AND week <= $endDate::TIMESTAMP AND flag_warn =1
            group by bras_id
  ...
  def listCritAlerts(week: String): Future[Seq[(String, Long)]] = {
    ...
    val endDate = week.split("->")(1).trim
    dbConfig.db.run(
      sql"""select bras_id, sum(crit) crit from dmt_weekly_ops where week >= $startDate::TIMESTAMP AND week <= $endDate::TIMESTAMP AND flag_crit =1 OR flag_cluster = 1
            group by bras_id
  ...
  def listWarnAlerts(week: String): Future[Seq[(String, Long)]] = {
    ...
    val endDate = week.split("->")(1).trim
    dbConfig.db.run(
      sql"""select bras_id, sum(warn) warn from dmt_weekly_ops where week >= $startDate::TIMESTAMP AND week <= $endDate::TIMESTAMP AND flag_warn = 1
            group by bras_id
            order by sum(warn) desc
            """
        .as[(String, Long)])
  ...
  def listSuyhao(week: String): Future[Seq[(String, Long, Long, Double)]] = {
    ...
    val endDate = week.split("->")(1).trim
    dbConfig.db.run(
      sql"""select host, sum(pass), sum(not_pass), sum(rate) from dmt_weekly_suyhao where week >= $startDate::TIMESTAMP AND week <= $endDate::TIMESTAMP AND flag_not_pass = 1
            group by host
            """
  ...
  def listBroken(week: String): Future[Seq[(String, Long, Long)]] = {
    ...
    val endDate = week.split("->")(1).trim
    dbConfig.db.run(
      sql"""select host, sum(lost_signal), sum(affected_client) from dmt_weekly_splitter where week >= $startDate::TIMESTAMP AND week <= $endDate::TIMESTAMP AND flag_top_splitter = 1
            group by host
            """
        .as[(String, Long, Long)])
  }
  ...
  def listOLT(week: String): Future[Seq[(String,String, Int, Int, Int, Int, Int, Int)]] = {
    ...
    dbConfig.db.run(
      sql"""select host, module, sum(user_down), sum(inf_down), sum(lofi_error), sum(sf_error), sum(rogue_error), sum(sf_module)
            from dmt_weekly_inf where week >= $startDate::TIMESTAMP AND week <= $endDate::TIMESTAMP AND flag_sf_error = 1 OR flag_cluster = 1

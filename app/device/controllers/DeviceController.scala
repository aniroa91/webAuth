    ...
    try {
      if (!formValidationResult.hasErrors) {
       ...
       // val kibanaDdos = Await.result(BrasService.getDdosResponse(brasId,day), Duration.Inf)
        ...
        Ok(device.views.html.search(form,username,BrasResponse(BrasInfor(numOutlier,(sigin,logoff)),KibanaOpviewByTime(kibanaBytime,opviewBytime),SigLogByTime(siginBytime,logoffBytime),
        ...
        Ok(device.views.html.search(form,username,null,CommonService.getCurrentDay()+"/"+CommonService.getCurrentDay(),null))
    }
    catch{
      case e: Exception => Ok(device.views.html.search(form,username,null,CommonService.getCurrentDay(),null))
    }
  }

  ...
  def index =  Authenticated { implicit request =>
    ...
    try {
      if (!formValidationResult.hasErrors) {
        ...
        Ok(views.html.profile.internet.compare.date.index(form, request.session.get("username").get.toString, contract,null, date,tpTime))
      } else {
        Ok(views.html.profile.internet.compare.date.index(form, request.session.get("username").get.toString, null, null,null,null))
      }
    } catch {
      case e: Exception => Ok(views.html.profile.internet.compare.date.index(form, request.session.get("username").get.toString, null,null,null, null))
    }
  }
}

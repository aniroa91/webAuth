  ...
  def index = Authenticated { implicit request =>
    ...
    try {
      if (!formValidationResult.hasErrors) {
        ...
        Ok(dns.views.html.search.index(form, second, response._1, logo, request.session.get("username").get.toString))
      } else {
        Ok(dns.views.html.search.index(form, null, null, null,request.session.get("username").get.toString))
      }
    }
    catch{
      case e: Exception => Ok(dns.views.html.search.index(form, null, null, null,request.session.get("username").get.toString))  }
  }
}

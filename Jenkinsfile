            ...
            steps {
                ...
                script {
                    docker.withRegistry('https://bigdata-registry.local:5043', '010ed969-34b5-473b-bcd9-01a207e7e382') {
                        def app = docker.build("${env.BRANCH_NAME}:${env.BUILD_ID}")
        ...
        stage('Deploying'){
            ...
            steps {
                sshagent (credentials: ['4a18733a-bef4-4220-84d7-30dd315c7483']) {
                    ...
                    //sh "ssh -o StrictHostKeyChecking=no root@172.27.11.161 'docker pull bigdata-registry.local:5043/${env.JOB_NAME}_${env.BRANCH_NAME}:${env.BUILD_ID}'"
                    //sh "ssh -o StrictHostKeyChecking=no root@172.27.11.161 'docker rm -f ${env.JOB_NAME}_${env.BRANCH_NAME} && docker run -d -p 9000:9000 -h ${env.JOB_NAME}_${env.BRANCH_NAME} --name ${env.JOB_NAME}_${env.BRANCH_NAME} -v /public/images/:/opt/bigdata-play/public/ bigdata-registry.local:5043/${env.JOB_NAME}_${env.BRANCH_NAME}:${env.BUILD_ID}'"
                    //sh "ssh -o StrictHostKeyChecking=no root@172.27.11.161 'docker run -d -p 9000:9000 -h ${env.JOB_NAME}_${env.BRANCH_NAME} --name ${env.JOB_NAME}_${env.BRANCH_NAME} -v /public/images/:/opt/bigdata-play/public/ bigdata-registry.local:5043/${env.JOB_NAME}_${env.BRANCH_NAME}:${env.BUILD_ID}'"
